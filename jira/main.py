...
def get_attr_jira(issue_code: str):
  dict_attr = {}
  print(f"********** Leyendo Ticket {issue_code} **********")
  issue = jira.issue(issue_code)

  # Mostrar todos los campos disponibles
  for field, value in vars(issue.fields).items():
    if value is not None:
      dict_attr[field] = value
  print(f"********** Ticket {issue_code} leido correctamente **********")
  return dict_attr

def get_attr_subtask(subtasks_code: str):
  print(f"********** Leyendo atributos de las subtasks **********")
  subtask_list = []
  ...
  for subtask in subtasks_code:
    aux_dicct = {}
    for key, value in get_attr_jira(subtask.key).items():
      if key in list_keys_validation:
        if key == 'parent':
          ...
          aux_dicct['team_backlog'] = vars(jira.issue(value[0]).fields)['customfield_13702']
        else:
          aux_dicct[key] = value
...
def clean_values(dict_jira_prev):
  ...
  for key, value in dict_jira_prev.items():
    ...
    if key == 'team_backlog':
      dict_final[key] = vars(jira.issue(value[0]).fields)['customfield_13702']
  return dict_final
...
def validate_ticket(username: str, token: str):
  ...
  jira=JIRA(options=jira_options, auth=(username,token))
  
  link_issue = input("Ingrese link de jira: ")
  issue_code = link_issue.split('/')[-1]
  subtasks_attr = get_attr_jira(issue_code)
  subtasks_code = subtasks_attr['subtasks']
  subtasks_value = get_attr_subtask(subtasks_code)

  print("********** Actualizando valor de subtask de la issue principal **********")
  subtasks_attr['subtasks'] = subtasks_value

  dict_jira_prev = rename_keys(subtasks_attr)
  dict_final = clean_values(dict_jira_prev)
